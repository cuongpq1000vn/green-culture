// CMS Wrapper Components
// These components fetch data from Strapi and pass it to the base components

export { HeroCMS } from './hero-cms';
export { StatsBarCMS } from './stats-bar-cms';
export { ProductsCMS } from './products-cms';
export { AboutCMS } from './about-cms';
export { CTACMS } from './cta-cms';
export { TestimonialsCMS } from './testimonials-cms';
export { FacilitiesCMS } from './facilities-cms';
export { ProcessCMS } from './process-cms';
export { GlobalPartnersCMS } from './global-partners-cms';
export { BlogCMS } from './blog-cms';